...
def build_net(conv2d1_num_filters=16):
    net = NeuralNet(
        layers=[
        ...
        conv2d1_num_filters=conv2d1_num_filters,
    ...
    #     maxpool2_pool_size=(2,2),
        ...
        dense1_num_units=conv2d1_num_filters,
...
net = build_net(conv2d1_num_filters=16)
nn = grid_search.GridSearchCV(net, {'conv2d1_num_filters':[16,32,64,128]}, verbose=10000, cv=2, n_jobs=4)

nn2 = nn.fit(X_train, Y_train)
print('training complete')

# In[11]:

print nn2.grid_scores_
# In[12]:
...
submission.to_csv('predicted_1_cross_val_cnn_epochs'+str(max_epochs)+'_valsplit'+str(validate_split)+'.csv',index=False)
...
pickle.dump(nn, open('./oneCNN.pickle', 'w'))
print('Pickle saved')
print('terminate')
